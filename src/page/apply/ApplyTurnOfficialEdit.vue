    ...
    <div>
		...
		<div class="m-applyReason">
			<div class="txt">
				...
				<textarea class="reason" name="cause" v-model="form.cause" placeholder="请输入申请理由"></textarea>
			</div>
		</div>
		<!-- 申请理由 end-->
...
export default {
	...
	methods:{
		...
		submit(){
            ...
            vm.loading = true
            this.axios.post(this.$store.state.httpUrl.apply.applyTurnOfficial + '/?id=' + vm.applyId,qs.stringify(postData))
            .then(function (res) {
                if (res.data.code == 1) {
                    vm.F['Hint'](vm,{
