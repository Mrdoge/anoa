    ...
    <div>
		...
		<div class="m-applyList">
			...
			<div class="list f-clear">
				...
				<div class="right j-setSext" @click="sexPicker.visible = true">
...
export default {
	...
	methods:{
		...
		submit(){
            ...
            .then(function (res) {
                if (res.data.code == 1) {
                    ...
                    setTimeout(() => {
                        vm.$router.push('/myIndex/myApplyList/0');   //我的请假申请
                    },1500)
		...
		setData(data){
			...
			vm.form.native_place = data.native_place
			vm.form.nation = data.nation
			vm.form.phone_mob = data.phone_mob
			vm.form.phone_mob = data.phone_mob
			vm.form.s_id = data.s_id
			vm.form.gender = data.gender
			vm.form.sp_id = data.sp_id
			vm.form.data_birth = data.data_birth
			vm.form.pass_user_id = data.pass_user_id

			//设置部门选中
			// for (let i = 0; i < vm.sectionPicker.items[0].values.length; i++) {
			// 	if (vm.sectionPicker.items[0].values[i] == data.s_name) {
			// 		vm.$set(vm.sectionPicker.items[0],'index',i)
			// 		break;
			// 	}
			// }
			vm.sectionPicker.nowValue = data.s_name

			//设置职位选中
			// for (let i = 0; i < vm.rolePicker.items[0].values.length; i++) {
			// 	if (vm.rolePicker.items[0].values[i] == data.sp_name) {
			// 		vm.$set(vm.rolePicker.items[0],'index',i)
			// 		break;
			// 	}
			// }
			vm.rolePicker.nowValue = data.sp_name
			
			//设置性别选中
			if (data.gender == 1) {	//男
				vm.sexPicker.nowValue = "男"
			}else{
				vm.sexPicker.nowValue = "女"
			}

			//设置日期选中
			var date = vm.F['unixFormat'](data.data_birth).split(' ')[0];
			date = date.replace(/-/g,'/')
			vm.datePicker.nowValue = date
			...
			//数据检测（检测该部门下有没有人员）
			for (let i = 0; i < vm.sectionData.length; i++) {
				if(data.s_id == vm.sectionData[i].s_id){
					if(!vm.sectionData[i].sp_data.length){
						vm.form.sp_id = ''
					}
				}
			}
